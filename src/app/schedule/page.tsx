...
export default function SchedulePage() {
  ...
  const loadSlots = async (uid: string) => {
    ...
    const [{ data: mine }, { data: salon }] = await Promise.all([
      ...
      supabase.from('slots').select('*').is('hairdresser_id', null).gte('date', from).lte('date', to).order('date').order('start_time'),
    ])
    setMySlots(mine || [])
    setSalonSlots(salon || [])
  }
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    await supabase.from('slots').insert({ hairdresser_id: userId, date: selectedDate, start_time: startTime + ':00', end_time: endTime + ':00', status: 'available' })
  ...
  const isOverlapping = (a: Slot, b: Slot) => a.start_time < b.end_time && a.end_time > b.start_time
    ...
    <div className="min-h-screen px-6 py-16" style={{ background: '#f7f4ef' }}>
      <div className="max-w-5xl mx-auto">
        <div className="flex items-end justify-between mb-16">
          <div>
            <p className="text-xs tracking-[0.3em] mb-3" style={{ color: '#a09890' }}>SCHEDULE</p>
            <h1 className="font-serif text-4xl" style={{ fontWeight: 300 }}>スケジュール管理</h1>
          </div>
          <button
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          {weekOffset !== 0 && (
            <button onClick={() => setWeekOffset(0)} className="text-xs underline underline-offset-4" style={{ color: '#6b7c5c' }}>今週</button>
          )}
        </div>
        ...
        <div className="border" style={{ borderColor: '#e2dcd4' }}>
          <div className="grid grid-cols-7 border-b" style={{ borderColor: '#e2dcd4' }}>
            {dates.map((date, i) => (
              <div key={i} className="py-3 text-center border-r last:border-r-0" style={{ borderColor: '#e2dcd4', background: date.toDateString() === new Date().toDateString() ? '#faf9f7' : 'transparent' }}>
            ...
            {dates.map((date, i) => {
              ...
              return (
                <div key={i} className="min-h-[180px] p-1.5 space-y-1 border-r last:border-r-0" style={{ borderColor: '#e2dcd4' }}>
                  {mine.map(slot => {
                    const overlap = salon.some(s => isOverlapping(slot, s))
                    return (
                      <div key={slot.id} className="group relative px-2 py-1.5 text-xs border" style={{ borderColor: overlap ? '#6b7c5c' : '#e2dcd4', background: overlap ? 'rgba(107,124,92,0.08)' : 'rgba(240,236,228,0.5)', color: overlap ? '#6b7c5c' : '#6b6459' }}>
                        {overlap && <span className="text-xs font-medium">◎ </span>}
                        {slot.start_time.slice(0, 5)}
                        {slot.status === 'available' && (
                          <button onClick={() => handleDelete(slot.id)} className="absolute top-0.5 right-0.5 opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: '#85403b' }}>
                            <Trash2 size={9} />
                          </button>
                        )}
                      </div>
                  ...
                  <button onClick={() => { setSelectedDate(date.toISOString().split('T')[0]); setShowModal(true) }} className="w-full py-1 text-xs border border-dashed transition-all hover:border-[#c9b99a]" style={{ borderColor: '#ede9e2', color: '#c9b99a' }}>
        ...
        <div className="flex gap-6 mt-4 text-xs" style={{ color: '#a09890' }}>
          <span>□ 自分の空き枠</span>
          <span style={{ color: '#6b7c5c' }}>◎ サロンとの重複（予約可能）</span>
        </div>
      </div>
